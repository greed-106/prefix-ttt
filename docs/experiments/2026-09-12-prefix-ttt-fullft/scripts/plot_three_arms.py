"""Figures and a machine-readable summary for the E0 / E2 / E2-full comparison.
...
COLORS = {'e0': 'tab:blue', 'e2': 'tab:red', 'e2full': 'tab:green'}
LABELS = {'e0': 'E0 (base LLaVA)', 'e2': 'E2 (LoRA + TTT)',
          'e2full': 'E2-full (full fine-tuning + TTT)'}
SCORES = {
...
def plot_score_vs_cost(scores, costs, images, reference='e0',
                       candidates=('e2', 'e2full')):
        ...
        for candidate in candidates:
            for task, values in scores.get(candidate, {}).items():
                ...
                for name, value in values.items():
                    ...
                    axis.annotate(f'{candidate}:{task}:{name}',
                                  (ratio[1] / ratio[0], 100 * value / base[name]),
                                  fontsize=6, textcoords='offset points', xytext=(4, 2))
        axis.axvline(1.0, color='grey', lw=.8, ls='--')
        axis.axhline(100.0, color='grey', lw=.8, ls='--')
        axis.set_xlabel(f'{xlabel}: candidate / {reference.upper()}')
        axis.legend(handles=[plt.Line2D([], [], color=COLORS[c], marker='o', lw=0,
                                        label=c.upper()) for c in candidates], fontsize=7)
        axis.grid(alpha=.3)
    axes[0].set_ylabel(f'official score / {reference.upper()} (%)')
