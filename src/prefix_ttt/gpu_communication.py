"""Short multi-rank, possibly multi-node NCCL correctness and timing check."""
import json
import os
import socket
import time
...
def main():
    ...
    for _ in range(10):
        dist.all_reduce(payload)
    torch.cuda.synchronize()
    print(json.dumps({'host': socket.gethostname(), 'rank': dist.get_rank(),
                      'local_rank': local_rank, 'device': torch.cuda.get_device_name(),
